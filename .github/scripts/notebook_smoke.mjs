...
await check('Время создания отображается', async () => {
  const created = row.locator('.task-created-time');
  await created.waitFor();
  if (!(await created.textContent())?.trim()) throw new Error('Время создания пустое');
});
...
await check('Первое действие', async () => {
  ...
  if (!box || box.width < 80) throw new Error(`Текущий шаг имеет ширину ${box?.width ?? 0}px`);
...
await check('Завершение и архив выполненных', async () => {
  await row.getByTitle('Отметить выполненной').click();
  const summary = page.getByRole('button', { name: /Выполнено · 1/ });
