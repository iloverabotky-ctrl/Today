...
function NotebookPage({ tasks, upcoming, activeTaskId, dueItems, now, dragId, setDragId, createTask, updateTask, addStep, updateStep, deleteStep, openTask, requestActivate, reorderNotebook, openReminder, moveToBoard, toggleCompleted, finishTask, showNow }: {
  ...
  return <main className="notebook-page v6-notebook">
    ...
    <form className="new-task-line v6-new-task" onSubmit={(event) => { event.preventDefault(); if (!newTask.trim()) return; createTask(newTask, project); setNewTask(''); }}><span>＋</span><input id="new-notebook-task" value={newTask} onChange={(event) => setNewTask(event.target.value)} placeholder="Новая задача..." /><ProjectPicker value={project} setValue={setProject} /><button>Добавить</button></form>
...
function NotebookRow({ number, task, now, active, waiting, onDragStart, onDrop, updateTask, addStep, updateStep, deleteStep, openTask, requestActivate, openReminder, moveToBoard, toggleCompleted, finishTask }: {
  ...
  const visible = task.steps.slice(-2); const hiddenSteps = task.steps.slice(0, -2); const hidden = hiddenSteps.length;
  return <article className={`notebook-row v6-row project-${task.project} ${waiting ? 'is-waiting' : ''} ${active ? 'active-row' : ''} ${task.notebookCompleted ? 'notebook-done' : ''}`} draggable onDragStart={onDragStart} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); onDrop(); }}>
    ...
    <div className="row-main">
      <div className="row-task-line"><button className={`notebook-check ${task.notebookCompleted ? 'checked' : ''}`} onClick={() => toggleCompleted(task.id)} title="Отметить выполненной">✓</button><span className="drag-handle" title="Перетащить">⋮⋮</span><button className="row-title row-title-open" type="button" onClick={() => openTask(task.id)} title="Открыть задачу">{task.title}</button><button className="open-task-button" onClick={() => openTask(task.id)} title="Открыть задачу">↗</button>{active && <span className="focus-badge">Сейчас</span>}<span className="project-mark">{projectLabel(task.project)}</span>{waiting && <button className="waiting-status" onClick={() => openReminder({ taskId: task.id })}>ЖДУ</button>}<span className="last-age">{ageLabel(lastTouched(task), now)}</span></div>
